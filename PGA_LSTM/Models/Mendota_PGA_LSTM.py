...
from tensorflow.keras.layers import Dropout, ELU
...
def read_Data(use_temporal_feature):
...
def train_subset(train_X, train_y, train_y_glm, tr_frac):
...
def createModel(
  ...
  mlstm_out = mLSTM(input_units = lstm_nodes, hidden_units = feedforward_nodes, output_units = 1, 
                    return_sequences = True, use_bias = lstm_bias, dropout = drop_frac, 
                    recurrent_dropout = drop_frac, hidden_dropout = drop_frac, name='aux_output')(main_input)
  ...
  activ_dense3 = ELU(alpha=1.0)(dense3)
...
#     tf.random.set_seed(iteration)
    ...
    #read Data
    train_X,train_y_temperature,train_y_glm,test_X,test_y_temperature,test_y_glm = read_Data(use_temporal_feature)
    ...
    #Create train subset
    ix, train_X, train_y_temperature, train_y_glm = train_subset(train_X, train_y_temperature, train_y_glm, tr_frac)
...
#     exp_name = exp_name.replace('.', 'pt')
    exp_name = f"pga_lstm_phyval_{use_GLM}_usetemp_{use_temporal_feature}_trfrac_{tr_frac}_lambda_{int(lamda_reg)}_{iteration}"
    
    results_dir = Path('../Results/Mendota_PGA_mLSTM/new_exp')
    results_dir.mkdir(exist_ok=True, parents=True)
    
    model_name = results_dir / f'{exp_name}_model.h5'  # storing the trained model
    results_name = results_dir / f'{exp_name}_results.mat'  # storing the results of the model
    
    if results_name.exists(): return
    ...
    early_stopping = EarlyStopping(
                            ...
                            verbose = 1)
    callbacks = [early_stopping, TerminateOnNaN(), TqdmCallback(verbose=1)]
...
def main():
    ...
    tr_frac_range = [5, 10,20,30,40,50,100];
...
#         Parallel(n_jobs=-1)(delayed(PGA_mLSTM_train_test)(iteration, tr_frac, val_frac, patience_val, 
#                              num_epochs, batch_size, lstm_nodes, feedforward_nodes,
#                              mask_value, drop_frac, lstm_bias, n_nodes, use_GLM, 
#                              use_temporal_feature, lamda_reg, shuffle, lamda_main, 
#                              lamda_aux, tol, pad_steps) for tr_frac in tr_frac_range)
        for tr_frac in tr_frac_range:
            PGA_mLSTM_train_test(iteration, tr_frac, val_frac, patience_val, 
                             num_epochs, batch_size, lstm_nodes, feedforward_nodes,
                             mask_value, drop_frac, lstm_bias, n_nodes, use_GLM, 
                             use_temporal_feature, lamda_reg, shuffle, lamda_main, 
                             lamda_aux, tol, pad_steps)
