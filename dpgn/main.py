...
#     edge_attr_size = 1    # embedding index
    ...
    if cfg['modelpath']:
        ...
        print("new model is initialized. {}".format(modeldir))
        print("random coefficients: {}, {}".format(model.gn.a, model.gn.b))
#         print(model)
    ...
    tr_ind, val_ind, te_ind = 250*tr_frac, 300, TIME_DIM-1    # training/validation/test split

    #### Training
    for iter_ in trange(num_iterations):
...
#             for vt in range(tr_ind, val_ind - num_processing_steps):
            for vt in range(250, val_ind - num_processing_steps):
